 ...
 */
import { Target, Clock, Users, TrendingUp, AlertCircle, Activity, AlertTriangle } from "lucide-react";
...
export const getRiskTypeIcon = (type: RiskType) => {
  ...
  switch (type) {
    case 'dependency_bottleneck': return Target(iconProps);
    case 'deadline_pressure': return Clock(iconProps);
    case 'resource_overload': return Users(iconProps);
    case 'scope_creep': return TrendingUp(iconProps);
    case 'blocked_tasks': return AlertCircle(iconProps);
    case 'unassigned_tasks': return Users(iconProps);
    case 'missing_estimates': return Activity(iconProps);
